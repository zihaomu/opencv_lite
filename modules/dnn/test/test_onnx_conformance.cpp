...
public:
    ...
    {
        parser_deny_list = {
            #include "test_onnx_conformance_denylist.inl.hpp"
        };
    }
...
TEST_P(Test_ONNX_conformance, Layer_Test)
{
    ...
    {
        applyTestTag(CV_TEST_TAG_DNN_SKIP_CPU, CV_TEST_TAG_DNN_SKIP_ONNX_CONFORMANCE);
    }
    ...
    std::vector<std::string> inputNames = net.getInputName();
    ...
    std::vector<std::string> layerNames = net.getOutputName();
    ...
    if (checkAccuracy)
    {
        try
        {
            if (ref_outputs.size() == 1)
            {
                ...
                // TODEL
                auto a = shape(ref_outputs[0]);
                auto b = shape(outputs[0]);
                auto aa = ref_outputs[0].type();
                auto ab = outputs[0].type();
