...
static void MatShapeToInt64(const MatShape& inShape, std::vector<int64>& outShape)
{
    ...
    if (outShape.empty() || outShape.size() != sizeLen)
...
{
    // ONNXRuntime
    // TODO add HAVE_ORT protected macro
...
static void mat2Tensor(Mat& m, const std::vector<int64_t>& expactedInt64, int index, Ort::MemoryInfo& memoryInfo, Ort::Value& t)
{
...
{
    int total = 1;
    for (int i = 0; i < int64Shape.size(); i++)
    {
        total *= int64Shape[i];
    }
    return  total;
...
void Net::Impl::setInput(InputArray blob_, const String& name)
{
