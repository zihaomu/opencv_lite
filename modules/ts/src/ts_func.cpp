...
norm_(const _Tp* src1, const _Tp* src2, size_t total, int cn, int normType, double startval, const uchar* mask)
{
    ...
    if( normType == NORM_INF )
    {
        if( !mask )
            for( i = 0; i < total; i++ )
                result = std::max(result, (double)std::abs((int)(src1[i] - src2[i])));
        else
            for( int c = 0; c < cn; c++ )
            {
                for( i = 0; i < total; i++ )
                    if( mask[i] )
                        result = std::max(result, (double)std::abs((int)(src1[i*cn + c] - src2[i*cn + c])));
            }
    }
    else if( normType == NORM_L1 )
    {
        if( !mask )
            for( i = 0; i < total; i++ )
                result += std::abs((int)(src1[i] - src2[i]));
        else
            for( int c = 0; c < cn; c++ )
            {
                for( i = 0; i < total; i++ )
                    if( mask[i] )
                        result += std::abs((int)(src1[i*cn + c] - src2[i*cn + c]));
            }
    }
    else
    {
...
double norm(InputArray _src1, InputArray _src2, int normType, InputArray _mask)
{
    ...
    // when output and ref_output is empty, just return.
    if (total == 0)
        return 0.0;
